...
def procesar_swaps(input_dir, output_dir):
    try:
        ...
        # === fecha_base como Timestamp normalizado (00:00) ===
        fecha_str = f"{a1}{m1}{d1}"
        fecha_base = pd.to_datetime(fecha_str, format="%Y%m%d")  # Timestamp
        fecha_base = fecha_base.normalize()                      # 00:00 sin tz
        ...
        # === Filtrar registros según fecha_cobro (comparación determinística) ===
        if 'fecha_cobro' in df_flujo.columns:
            # Limpieza básica y parseo a datetime64[ns]
            col = df_flujo['fecha_cobro'].astype(str).str.strip().str.replace(r'\s+', ' ', regex=True)
            df_flujo['fecha_cobro'] = pd.to_datetime(
                col,
                format='%d/%m/%Y',    # si el formato puede variar, usar dayfirst=True
                errors='coerce'
            )

            # Si llegara con tz en algún origen, elimina la zona
            if pd.api.types.is_datetime64tz_dtype(df_flujo['fecha_cobro']):
                df_flujo['fecha_cobro'] = df_flujo['fecha_cobro'].dt.tz_convert(None)

            # Normaliza a 00:00 para comparar "por fecha" (equivalente a tu .dt.date)
            df_flujo['fecha_cobro'] = df_flujo['fecha_cobro'].dt.normalize()

            # Métricas y filtro
            filas_antes = df_flujo.shape[0]
            mask = df_flujo['fecha_cobro'] > fecha_base
            df_flujo = df_flujo.loc[mask].copy()
            filas_despues = df_flujo.shape[0]

            logging.info(
                f"Filtrado por fecha: se eliminaron {filas_antes - filas_despues} "
                f"filas con fecha_cobro <= {fecha_base.date()}"
            )

            # Aviso si hubo fechas inválidas en el parseo
            n_nat = df_flujo['fecha_cobro'].isna().sum()
            if n_nat:
                logging.warning(f"Se encontraron {n_nat} filas con fecha_cobro inválida (NaT) tras el parseo.")
